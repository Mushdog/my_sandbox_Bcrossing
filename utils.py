...
def get_dfs(dataset):
    ...
    elif dataset == 'bcrossing':
        users_path = ratings_path.replace('ratings.', 'users.')
        movies_path = ratings_path.replace('ratings.', 'books.')
        dfs = bcrossing_to_df(ratings_path, users_path, movies_path)
    else:
...
def bcrossing_to_df(ratings_file, users_file, movies_file):
    """
        Takes a dataset string and return that data in a dataframe!
    """
    ratings_names = ["user_id","movie_id","rating"]
    ratings_delim = ','
    users_names = ["user_id", "age", "country","state"]
    users_delim = ','
    movies_names = ["isbn", "book_title", "book_author", "year_of_publication", "movie_id"]
    movies_delim = ','
    encoding = 'utf-8'
    engine = 'python'

    ratings_df = pd.read_csv(ratings_file,names=ratings_names,skiprows=1,sep=ratings_delim)
    users_df = pd.read_csv(users_file,names=users_names,skiprows=1,sep=users_delim)
    movies_df = pd.read_csv(movies_file,names=movies_names,skiprows=1, sep=movies_delim)
...
if __name__ == '__main__':
    print('Testing...')
    assert extract_from_filename("dataset-bcrossing_type-gender_userfrac-1.0_ratingfrac-1.0", "userfrac-", 3) == '1.0'
    assert extract_from_filename("dataset-test_bcrossing_type-gender_userfrac-1.0_ratingfrac-1.0", "userfrac-", 3) == '1.0'
    assert extract_from_filename(
        "dataset-test_bcrossing_type-gender_userfrac-1.0_ratingfrac-1.0", "dataset-", None, '_type') == 'test_ml-1m'
    assert extract_from_filename("dataset-bcrossing_type-gender_userfrac-1.0_ratingfrac-1.0", "ratingfrac-", 3) == '1.0'
    assert extract_from_filename(
        "dataset-bcrossing_type-sample_users_userfrac-1.0_ratingfrac-1.0_sample_size-1_num_samples-250_indices-251-to-500.csv",
        "indices-", 3) == '251'
    assert extract_from_filename(
        "dataset-bcrossing_type-sample_users_userfrac-1.0_ratingfrac-1.0_sample_size-1_num_samples-250_indices-251-to-500.csv",
